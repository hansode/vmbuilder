...
function test_render_fstab_ptab_ext3() {
  ...
  render_fstab_ptab ${chroot_dir} ${disk_filename} | egrep -w / | egrep -q -w ${preferred_filesystem}
  assertEquals $? 0
}
...
function test_render_fstab_ptab_type_undefined() {
  ...
  render_fstab_ptab ${chroot_dir} ${disk_filename} | egrep -w / | egrep -q -w ext3
  assertEquals $? 0
}
...
function test_render_fstab_ptab_type_uuid() {
  ...
  render_fstab_ptab ${chroot_dir} ${disk_filename} | egrep -w / | egrep -q ^UUID=
  assertEquals $? 0
}
...
function test_render_fstab_ptab_type_label() {
  ...
  render_fstab_ptab ${chroot_dir} ${disk_filename} | egrep -w / | egrep -q ^LABEL=
  assertEquals $? 0
}
...
function test_render_fstab_ptab_type_unknown() {
  ...
  render_fstab_ptab ${chroot_dir} ${disk_filename} >/dev/null 2>&1
  assertNotEquals $? 0
}
